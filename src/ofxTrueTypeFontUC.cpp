...
// Via https://github.com/hironishihara/ofxTrueTypeFontUC/issues/18#issuecomment-192666469
...
public:
  ...
  std::vector<charPropsUC> cps;  // properties for each character
...
static ofPath makeContoursForCharacter(FT_Face & face) {
  ...
  for(int k = 0; k < nContours; k++){
    ...
    //std::vector <ofPoint> testOutline;
    ofPoint lastPoint;
...
//-----------------------------------------------------------
std::vector<ofPath> ofxTrueTypeFontUC::getStringAsPoints(const std::string &src, bool vflip){
...
void ofxTrueTypeFontUC::Impl::implReserveCharacters(int num) {
  ...
  std::vector<charPropsUC>().swap(cps);
  std::vector<ofTexture>().swap(textures);
  std::vector<int>().swap(loadedChars);
  std::vector<ofPath>().swap(charOutlines);
...
void ofxTrueTypeFontUC::Impl::loadChar(const int &charID) {
  ...
  #if (OF_VERSION_MAJOR == 0 && OF_VERSION_MINOR >= 9 && OF_VERSION_PATCH >= 2) || (OF_VERSION_MAJOR == 0 && OF_VERSION_MINOR >= 10) || OF_VERSION_MAJOR > 0
